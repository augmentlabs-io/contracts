...
import {IAugmentPair} from "../interfaces/IAugmentPair.sol";
import {IAugmentRouter01} from "../interfaces/IAugmentRouter01.sol";
import {IWETH} from "../interfaces/IWETH.sol";
import {Babylonian} from "../libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by AugmentLab teams.
 */
contract AugmentZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // AugmentRouter interface
    ...
    // Address AugmentRouter
    address private augmentRouterAddress;
     ...
     * @param _augmentRouter: address of the AugmentRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _augmentRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        augmentRouterAddress = _augmentRouter;
        augmentRouter = IAugmentRouter01(_augmentRouter);
    ...
    {
        address token0 = IAugmentPair(_lpToken).token0();
        address token1 = IAugmentPair(_lpToken).token1();
        ...
        (uint256 reserveA, uint256 reserveB, ) = IAugmentPair(_lpToken)
            .getReserves();
        ...
        if (token0 == _tokenToZap) {
            ...
            swapAmountOut = augmentRouter.getAmountOut(
                swapAmountIn,
                reserveA,
                reserveB
            );
    ...
    {
        require(
            _token0ToZap == IAugmentPair(_lpToken).token0() ||
                _token0ToZap == IAugmentPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IAugmentPair(_lpToken).token0() ||
                _token1ToZap == IAugmentPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IAugmentPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            ...
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) /
                IAugmentPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = augmentRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = augmentRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IAugmentPair(_lpToken).burn(
            address(this)
        );
        ...
        // Swap tokens
        augmentRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (
            IERC20(_token).allowance(address(this), augmentRouterAddress) < 1e24
        ) {
            // Re-approve
            IERC20(_token).safeApprove(augmentRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = augmentRouter.getAmountOut(
            halfToken0Amount,
            _reserve0,
            _reserve1
        );
        uint256 denominator = augmentRouter.quote(
            halfToken0Amount,
